...
from splits import EMO4_NAMES, EMO7_NAMES
...
def load_model(ckpt_path, device):
    ...
    model = EmotionGAT(
        ...
        n_classes=arch.get("n_classes", 4), d_model=arch.get("d_model", 128),
...
def generate(ckpt_path, test_path, out_dir, title="EmoDB in-corpus", device=None):
    ...
    png = out_dir / f"cm_emodb_incorpus_{n_classes}class.png"
    plot_cm(cm, names, png, f"{title} {n_classes}-class  (WF1 {wf1:.1f})")

    tsv = out_dir / f"predictions_emodb_incorpus_{n_classes}class.tsv"
...
def main():
    ...
    p.add_argument("--classes", type=int, choices=[4, 7], default=4)
    p.add_argument("--ckpt", default=None)
    p.add_argument("--test", default=None)
    p.add_argument("--out_dir", default=str(HERE / "results"))
    args = p.parse_args()
    ckpt = args.ckpt or str(HERE / f"model_{args.classes}class" / "best.pt")
    test = args.test or str(HERE / "features" / f"emodb{args.classes}_test.jsonl")
    generate(ckpt, test, args.out_dir, title=f"EmoDB in-corpus ({args.classes}-class)")
