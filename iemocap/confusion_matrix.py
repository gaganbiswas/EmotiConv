...
CLASS_NAMES = {
    6: ["neutral", "frustrated", "angry", "sad", "happy", "excited"],
    4: ["neutral", "happy", "angry", "sad"],
}
...
def load_model(ckpt_path, device):
    ...
    model = EmotionGAT(
        ...
        n_classes=arch.get("n_classes", 6), d_model=arch.get("d_model", 128),
        heads=arch.get("heads", 8), n_layers=arch.get("layers", 4),
        past_window=arch.get("past_window", 6), future_window=arch.get("future_window", 0),
...
def generate(ckpt_path, test_path, out_dir, title="IEMOCAP", device=None):
    ...
    png = out_dir / f"cm_iemocap_{n_classes}class.png"
    plot_cm(cm, names, png, f"{title} {n_classes}-class  (WF1 {wf1:.1f})")

    tsv = out_dir / f"predictions_iemocap_{n_classes}class.tsv"
...
def main():
    ...
    p.add_argument("--title", default="IEMOCAP")
    args = p.parse_args()
