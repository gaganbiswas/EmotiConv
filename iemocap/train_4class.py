...
def build_model(d_audio, d_text, device):
    return EmotionGAT(d_audio=d_audio, d_text=d_text, n_classes=N_CLASSES).to(device)
...
class DialogDataset(Dataset):
    ...
    @classmethod
    def from_dialogs(cls, dialogs):
        obj = cls.__new__(cls)
        obj.dialogs = list(dialogs)
        return obj
...
def main():
    ...
    p.add_argument("--train", default=str(HERE / "features" / "iemocap_features_4class_train.jsonl"))
    p.add_argument("--test", default=str(HERE / "features" / "iemocap_features_4class_test.jsonl"))
    p.add_argument("--config", default=str(HERE / "config.json"))
    p.add_argument("--out_dir", default=str(HERE / "model_4class"))
    p.add_argument("--epochs", type=int, default=50)
    p.add_argument("--batch_size", type=int, default=4)
    p.add_argument("--lr", type=float, default=3e-4)
    p.add_argument("--weight_decay", type=float, default=5e-4)
    p.add_argument("--w_consistency", type=float, default=0.015)
    ...
    full = DialogDataset(args.train)
    idx = np.arange(len(full.dialogs))
    np.random.RandomState(VAL_SPLIT_SEED).shuffle(idx)
    n_val = max(1, int(round(VAL_FRACTION * len(idx))))
    val_ds = DialogDataset.from_dialogs(full.dialogs[i] for i in idx[:n_val])
    train_ds = DialogDataset.from_dialogs(full.dialogs[i] for i in idx[n_val:])
    ...
    model = build_model(d_audio, d_text, device)
    ...
    try:
        ...
        confusion_matrix.generate(str(best_path), args.test, args.out_dir, title="IEMOCAP")
